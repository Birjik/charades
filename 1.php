...
?>		
	<div class = "message">
	        В ы уверены что хотите закончить игру?<br>
	...
	<body onload="timer()">
		<div id="all">
			...
			<div id="content">
				<div id="finish">
				<span id="str">
				јвтор: <?php echo $author;?>
				</span>
	        	</div>
				<span id="str">Тур 1 (»гра є<?php echo $id;?>)</span>
				<hr>
				<div id="center">
				    <?php if($left == -1){?>
				    <br><br><br>				   
					<span style="color:grey; font-size:200%">Ожидание команды : <span style="color:green"><?php echo $name;?></span> (є<?php echo $cur;?>)</span>
				    <br><br><br>
					<span style="font-size:125%">ќсталось <?php echo $cnt;?> слов.</span>
					<br><br><br>
					<div id="add_word">
						<form action="lib/upd.php" method="post">
							<input name="next" type="submit" value="»грать!">
							&nbsp;
							<input name="endr" type="submit" value="«акончить тур">
						</form>
					</div>
					<?php }
					else {?>
					...
					<form action="lib/upd.php?play_time=1" method="post">
						...
						<?php
							$left = file_get_contents("style/left.txt");
							if($left == 0){
								echo "<span id='zero'>—ледующий [0]</span>";													
							}
							else {
						?>
